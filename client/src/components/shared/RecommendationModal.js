...
class RecommendationModal extends Component {
  ...
  constructor() {
    super();
    this.request_recommendation = this.request_recommendation.bind(this);
  }
      ...
      return (
        <div>
         <Modal isOpen={this.state.modal} toggle={this.toggle} size="lg" side position="bottom-right">
            <ModalHeader toggle={this.toggle}>FoodRoad Sistem de Recomandare</ModalHeader>
            <ModalBody>
            <div className="row">
            ...
            <ModalFooter>
              <Button color="secondary" onClick={this.request_recommendation}>Urmatoarea</Button>
              <Button color="primary" onClick={() => {this._addToCart(this.state.product.id, this.state.product.image, this.state.product.price, this.state.product.name, this.state.product.description, 1, this.state.product.restaurant_id)}}>Comanda</Button>
            </ModalFooter>
          </Modal>
        </div>
      )
